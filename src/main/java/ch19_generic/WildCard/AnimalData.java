...
public class AnimalData<T> {
    private T animal;
    /*
     현재 AnimalData의 필드인 animal에 Animal클래스의 상속을 받은
     Tiger / Human 객체를 필드에 대입할겁니다.

     근데 T에는 어떤 클래스도 들어갈 수 있기 때문에 현재 상황에서는
     Car 클래스의 객체도 animal 필드에 들어갈 수 있습니다.

     저희의 이번 목표는
     특정 클래스라면 (즉 Animal 클래스의 상속을 받은 서브 클래스라면)
     해당 클레스에 맞는 객체 정보를 출력하도록 할것
    */

    public void showData() {
        ((Animal) animal).move(); // 앞부분의 Animal은 형변환 부분 / animal 은 필드 이름
        // AnimalData의 필드인 animal을 Animal타입으로 형변환해서 .move() 메서드를 호출
