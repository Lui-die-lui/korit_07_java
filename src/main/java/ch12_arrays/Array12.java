...
/*
    배열의 출력
    엄밀히 말하자면 저희는 배열을 출력한게 아니라 여태까지 배열 내부의 element들을 출력
    System.out.println(arr01); 과 같은 출력문을 작성했을 때
    주소값만 나온다는 사실을 확인할 수 있었습니다.

    근데 element의 값을 조정하거나, 더하거나와 같은 추가적인 가공을 하기 위해  for반복문을 사용해왔는데,
    굳이 변형이 없고 배열 자체를 확인하고 싶을 때마다
    main에 반복문을 작성하든지 method를 정의하든지 하는 일은 번거로운 일이 됨
...
//        System.out.println(num2);
        ...
        // 배열의 전체 출력 예시
        // Arrays = class
        System.out.println(Arrays.toString(numbers)); // String 배열로 바뀜

        Method01.call1();
        System.out.println(Method01.call3());

        Integer[] nums = {3, 5, 6, 7, 1, 9, 2, 10, 5, 4, 8};
        System.out.println("정렬 전 배열 : " + Arrays.toString(nums));
        // 오름차순 정렬
        Arrays.sort(nums); // 결과값이 void니까 원본배열 바꿈 - 주소지 전체를 바꿈
        ...
        /*
           ...
           이상의 과정이 overloading이 이루어졌다는 점을 파악할 수 있으면
           현재 수준으로는 만족.

           그리고 두 번째 argument로 Comparator.reverseOrder(); 가 사용되는데,
           ...
           Integer[] 배열을 사용하ㅣ는데, 현재는
           char -> String 처럼 int -> Integer 라고만 생각하면 됨
        */
    }
