...
/*
    ...
    객체 생성 예시
    클래스명        객체명 = new 클래스명(); / 괄호는 필드값(속성 매개변수)
    ...
    특징 :
        1. 클래스의 이름과 생성자의 이름은 동일
            (다른 메서드들과 달리 대문자로 시작)
        2. return 값이 없다 -> 즉, call1() / call2() 유형으로 생각 가능
...
public class Constructor {
    ...
    // 기본 생성자는 call1() 유형에 해당할 것이고 , 클래스명과 동일한 메서드명을 가짐
    Constructor() {
        System.out.println("NoArgsConstructor(기본 생성자)");
    }

    // 매개변수 생성자의 정의 -> 기본적으로 만들어지지 않음. 개발자가 정의
    // 근데 매개변수 생성자가 하나라도 만들어지면 default로 만들어진 기본 생성자가 사라지기 때문에
    // 기본 생성자와 매개변수 생성자를 둘 다 사용하고 싶다면
    // 기본 생성자도 따로 '명시적으로 정의' 해야함.
    // 오버로딩이 적용되어있는 사례
    Constructor(int number) {
        System.out.println("RequiredArgsConstructor(int 매개변수를 필수로 요구하는 생성자)");
        this.num = number; // this : 해당 클래스에서 객체를 생성하게 되면 객체를 이름으로 대체함.
    }

    // String title을 매개변수로 하는 생성자 정의
    // RequiredArgsConstructor(String 매개변수를 필수로 요구하는 생성자)
    // 라는 안내문을 출력할 수 있도록 작성한 뒤,
    // ConstructorMain으로 가서, constructor3이라는 객체명을 지니고, title에 내 이름
    // 지어 객체를 생성한 후,
    // sout(constructor3.name);입력하고 콘솔에 내 이름 출력
