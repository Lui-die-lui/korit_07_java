...
//        int age = scanner.nextInt(); // 전에는 여기에 바로 작성해서 넣었는데,
        // 생각해보면 여기 부분에 음수값이나 200초과 정수를 집어넣을 수 있다.
        // 혹은 double 자료형을 넣을 수도 있음
        // 예외가 발생할 가능성이 있는 부분을 try문 안에 넣어주는것이 중요
        try {
        ...
        } catch (InputMismatchException e) { // Scanner 사용했는데  변수 자료형과 일치하지 않을때 적용됨
            // 스무살, 38.7 이 동일한 예외 발생
        ...
        * 예외 발생하지 않으면 예외 관련 객체가 생성 안됨
        * catch{} 문도 실행 안됨
        * 예외 발생 시점 이휴의 try문은 실행 되지 않고 바고 catch문이 실행
        * 그리고 예외 발생과 관계없이 finally는 실행됨
        * */
